...
from utils import set_seed, setup_muon_optimizer        
from model import MinimalLLM
from torch.amp import GradScaler
from torch.amp import autocast

def train_moe_model(config: ModelConfig, train_loader: DataLoader, val_loader: DataLoader):
    """Train the MoE model"""
    print(f"\n🚀 Training MoE model with {config.num_experts} experts (top-{config.top_k})")

    # Initialize model
    set_seed(42)
    cache = False
    model = MinimalLLM(cache, config)
    ...
    while step < config.max_steps:
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            if config.use_amp:
                with autocast():
                    logits, aux_loss = model(x, return_aux_losses=True)
